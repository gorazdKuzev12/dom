...
import { GET_AGENCY, GET_AGENCY_LISTINGS, GET_USER, GET_USER_LISTINGS } from './queries';
...
export async function getServerUserData() {
  try {
    ...
    if (!token) {
      ...
      return { user: null, listings: null, isAuthenticated: false, error: null };
    }

    // Decode JWT to get user ID (basic decode, server will verify the token)
    const payload = JSON.parse(atob(token.split('.')[1]));
    
    // Check if token is expired
    if (payload.exp && payload.exp < Date.now() / 1000) {
      return { user: null, listings: null, isAuthenticated: false, error: 'Token expired' };
    }

    // Fetch user data and listings from GraphQL with token in headers
    ...
    console.log('📊 Server-side user data fetch results:', {
      hasUser: !!userResult.data?.user,
      listingsCount: listingsResult.data?.userListings?.length || 0
    });

    return { 
      user: userResult.data?.user || null,
      listings: listingsResult.data?.userListings || [],
      isAuthenticated: true, 
      error: null 
    };
